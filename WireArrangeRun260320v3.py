...
# """ 輸出座標 Excel 試算表報表 """
    def export_coordinates(self, type):
        ...
        template_path = resource_path("Assets/template.xlsx") # 模板檔案
        is_template = False
        if os.path.exists(template_path):
            try:
                wb = load_workbook(template_path)
                ws = wb.worksheets[0] # 優先抓第一張表
                ws.title = sheet_name
                is_template = True
            except Exception:
                wb = Workbook()
        ...
        if is_template:
            ...
            if ws._charts:
                ...
                # 更新第一個序列的數據範圍
                if chart.series:
                    chart.series[0].xVal.numRef.f = f"'{sheet_name}'!$B${start_row}:$B${end_row}"
                    chart.series[0].yVal.numRef.f = f"'{sheet_name}'!$C${start_row}:$C${end_row}"
                # 同時確保圖表標題符合當前語言
                ...
                run = RegularTextRun(t=self.get_msg("chart_title", "排線路徑座標圖"))
                rtp = Paragraph(pPr=pp, r=[run])
                chart.title.tx.rich.p = [rtp]
                chart.title.overlay = False
                run = RegularTextRun(t=self.get_msg("axis_x", "X 座標 (mm)"))
                chart.x_axis.title.tx.rich.p = [rtp]
                run = RegularTextRun(t=self.get_msg("axis_y", "Y 座標 (mm)"))
                chart.y_axis.title.tx.rich.p = [rtp]
            ...
            cp = CharacterProperties(sz=1600, b=False, latin=font_style, ea=font_style) 
            pp = ParagraphProperties(defRPr=cp)
            text = self.get_msg("chart_title", "排線路徑座標圖")
            run = RegularTextRun(t=text)
            rtp = Paragraph(pPr=pp, r=[run])
            if hasattr(chart.title, 'tx'):
                chart.title.tx.rich.p = [rtp]
            else:
                # 針對沒有 tx 屬性的情況（如直接賦值給 title）
                chart.title = text
                chart.title.tx.rich.p = [rtp]

            text = self.get_msg("axis_x", "X 座標 (mm)")
            run = RegularTextRun(t=text)
            rtp = Paragraph(pPr=pp, r=[run])
            if hasattr(chart.x_axis.title, 'tx'):
                ...
                chart.x_axis.title.tx.rich.p = [rtp]    
            text = self.get_msg("axis_y", "Y 座標 (mm)")
            run = RegularTextRun(t=text)
            rtp = Paragraph(pPr=pp, r=[run])
            if hasattr(chart.y_axis.title, 'tx'):
                ...
                # 針對沒有 tx 屬性的情況（如直接賦值給 title）
                chart.y_axis.title = text
                chart.y_axis.title.tx.rich.p = [rtp] 

            
            chart.height = 18  # 設定高度為 10 公分
            chart.width = 22
            chart.layout = Layout(
                ...
                h=0.75,      # 高度比例75%
                w=0.85,      # 寬度比例85%
                xMode="edge", #edge: 表示座標是從圖表的最邊緣開始計算
                yMode="edge",
                layoutTarget="inner" # 確保文字不影響繪圖區大小
                )
            )
            # --- 2. 強制將刻度標籤固定在最邊緣 ('low') ---
            # 'low' 確保 X 軸數字在最下方，Y 軸數字在最左方
            chart.x_axis.tickLblPos = 'low' 
            chart.y_axis.tickLblPos = 'low'
            # 設定數字刻度為 10pt (sz=1000)
            tick_font_props = CharacterProperties(sz=1000) 
            tick_para_props = ParagraphProperties(defRPr=tick_font_props)
            
            # 賦予 X 軸與 Y 軸
            chart.x_axis.txPr = RichText(p=[Paragraph(pPr=tick_para_props, endParaRPr=tick_font_props)])
            chart.y_axis.txPr = RichText(p=[Paragraph(pPr=tick_para_props, endParaRPr=tick_font_props)])
            ...
            # X 軸數字格式：一位小數，負數顯示為紅色
            chart.x_axis.number_format = '#,##0.0_);[Red]\(#,##0.0\)' 
            # 2. Y 軸設定 (ID: 242071808)
            ...
            chart.y_axis.majorGridlines.spPr = GraphicalProperties(ln=line_props)
            # 設定格線寬度為 6350 EMUs (約 0.5 pt)
            line_props = LineProperties(w=6350)
            chart.y_axis.majorGridlines.spPr = GraphicalProperties(ln=line_props)
            # 這裡將數列加入圖表
            chart.series.append(series)
            # 6. 將圖表插入工作表 (例如放在 E2 旁邊)
