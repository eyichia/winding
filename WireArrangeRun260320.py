...
# """ 輸出座標 Excel 試算表報表 """
    def export_coordinates(self, type):
        ...
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = self.get_msg("coordinates_title", "座標摘要")
        ...
        # 2. 建立散佈圖物件
        chart = ScatterChart()
        chart.scatterStyle = 'lineMarker'  # 對應 <c:scatterStyle val="lineMarker"/>
        # 1. 定義字體樣式 (對應 XML 的 sz="1600" 與 typeface)
        # sz 的單位是 1/100 pt，所以 1600 = 16pt
        ...
        # 3. 建立標題的富文本結構
        # 我們建立一個段落 (Paragraph)，並將文字放入其中的 Run (r)
        rtp = Paragraph(pPr=pp, r=[RichText(t="排線路徑座標圖")])
        chart.title.tx.rich.p.append(rtp)
        # 4. 確保不覆蓋圖表 (對應 XML 的 <c:overlay val="0"/>)
        chart.title.overlay = False


        chart.roundedCorners = True        # 對應 <c:roundedCorners val="1"/>
        chart.style = 2  # 選擇一個內建樣式
        rtp = Paragraph(pPr=pp, r=[RichText(t="X 座標 (mm)")])
        chart.x_axis.title.tx.rich.p.append(rtp)
        rtp = Paragraph(pPr=pp, r=[RichText(t="Y 座標 (mm)")])
        chart.y_axis.title.tx.rich.p.append(rtp)
        #chart.x_axis.title = 'X 座標 (mm)'
        #chart.y_axis.title = 'Y 座標 (mm)'



        # 3. 定義數據範圍 (B10~B110 為 X, C10~C110 為 Y)
        # min_row=10, max_row=110, min_col=2 (B欄), min_col=3 (C欄)
        ...
        series = Series(y_values, x_values)#, title="排線路徑")
        # --- 重要設定：取消平滑線 ---
        series.smooth = False
        # --- 設定序列樣式 (座標線條) ---
        # 顏色 #2E75B6, 寬度 19050 EMUs (約 1.5 pt)
        series.graphicalProperties.line.solidFill = "2E75B6" 
        series.graphicalProperties.line.width = 19050 # 1.5pt 12700=1pt       
        series.graphicalProperties.line.prstDash = "solid"
        # --- 設定序列樣式 (標記/點) ---
        # 菱形 (diamond), 大小 7
        series.marker = Marker(symbol='circle', size=5)
        series.marker.graphicalProperties.line.prstDash = "solid"
        # 針對索引 100 的點單獨設定
        series.dLbls = DataLabelList() # ✅ 必須先建立這個物件，否則它是 None
        last_index = row_count - 1
        if last_index >= 0:
            specific_label = DataLabel(idx=last_index)
            specific_label.showVal = True       
            specific_label.showSerName = True   
            specific_label.dLblPos = 'r'        
            series.dLbls.dLbl.append(specific_label)
        chart.series.append(series)
        # 4. 設定座標軸 (X 軸 - valAx 14597120)
        chart.x_axis.number_format = '0.0' # '#,##0.0_);[Red]\(#,##0.0\)' # 對應 XML 格式
        ...
        chart.x_axis.minorUnit = 0.1
        

        # 5. 設定座標軸 (Y 軸 - valAx 239315200)
        chart.y_axis.number_format = '0.0'
        chart.y_axis.majorGridlines = ChartLines() # 建立物件即代表開啟格線
        ...
        chart.y_axis.minorUnit = 0.1
        
        # 6. 將圖表插入工作表 (例如放在 E10 旁邊)
