...
# """ 輸出橫向對比報表 (CSV 格式) """
    def export_summary_csv(self):
        ...
        try:
            ...
            show_prompt_window(self, self.get_msg("success"), 
                               f"{self.get_msg('csv_save_ok')}<br>{file_path}", 
                               self.logo_pixmap, theme="success")
        except Exception as e:
            show_prompt_window(self, self.get_msg("fail"), f"Export Error: {str(e)}", theme="alarm")
# """顯示軟體資訊視窗，且只改變此視窗的字型"""
