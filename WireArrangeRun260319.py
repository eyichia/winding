...
# """ 輸出座標 Excel 試算表報表 """
    def export_coordinates(self, type):
        ...
        # 1. 定義數據範圍 (B10~B110 為 X, C10~C110 為 Y)
        ...
        # 設定線條粗細，單位是 EMUs (1 pt = 12700 EMUs)，1.5pt 大約是 19050
        series.graphicalProperties.line.width = 19050
        # 設定標記點內部的填色
        series.marker.graphicalProperties.solidFill = line_color
        # 設定標記點外框的顏色 (確保不會出現預設的白邊或黑邊)
        series.marker.graphicalProperties.line.solidFill = line_color
        # 3. 設定為「直線與資料標記」
        # 在 openpyxl 中，ScatterChart 預設通常只有點
        # 我們可以透過設定數列的標記符號來確保它顯示點
        series.marker = Marker('diamond', 7) # 點的形狀為菱形
        # 確保線條顯示 (預設散佈圖可能不帶線，需確認圖表類型或手動開啟)
        chart.height = 10  # 設定高度為 10 公分
        chart.width = 15   # 設定寬度為 15 公分
        chart.x_axis.scaling.min = 0    # X軸最小值
        chart.x_axis.scaling.max = 12   # X軸最大值 (視你的機構件大小而定)
        chart.y_axis.scaling.min = 0
        chart.y_axis.scaling.max = 15
        chart.x_axis.majorUnit = 2        # 每 2mm 顯示一個數字
        chart.x_axis.number_format = '0.0' # 刻度顯示到小數第一位
        chart.x_axis.majorTickMark = "out" # 刻度線向外突出
        chart.y_axis.majorUnit = 5        # Y 軸間距較大，設為 5mm
        chart.y_axis.number_format = '0.0'
        chart.y_axis.majorTickMark = "out"
        # 這裡將數列加入圖表
        chart.series.append(series)

        # 4. 將圖表插入工作表 (例如放在 E10 旁邊)
